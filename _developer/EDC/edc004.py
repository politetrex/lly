'''
**Keyless and improved version** of edc002.  
edc003 is not published due to  
it's long recursion.  
'''
...
def greekd(string, thick):
    ...
    for i in range(thick):
        for j in range(len(string)//thick):
            ...
            cnt+=1
    fres = ''.join(res)
    while(fres[len(fres)-1]=='Q'):
        fres=fres[:len(fres)-1]
    return fres
...
def decode(_ecd):
    ...
    if key[0:5]=="EDCAD" or key[0:5]=="EDCAB":
        cmd_str = key[5:]
        dcd = ecd
        i = 0
        while i < len(cmd_str):
...
def encode(dcd, deep=10, check=True):
    ...
    while True:
        ...
        while _deep:
            ...
            prev=command
        key_="EDCAD"+key_
        if (decode(key_+'V'+dcd2)==dcd) or(not check):
            _key=key_
            _dcd=dcd2
            break
